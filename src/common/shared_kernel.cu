...
/**
 * Load the Irow training sample into the d_kernelrow with parrellism
...
template <unsigned int blockSize, bool isFeaturePow2>
											...
											int nfeatures)
{
	...
	while (i < nfeatures)
	{
		...
		if (isFeaturePow2 || i + blockSize < nfeatures)
		{
