...
/**
 * Performs a reduction to sum the rbf values
 * @param d_ytraindata device pointer to the array with binary labels
									...
									int kernelcode)
{
	...
	while (i < ntraining)
	{
		...
		if(kernelcode==0)
		{
			...
			reduction[tid]+=d_atraindata[bidy*ntraining+i]*d_ytraindata[bidy*ntraining+i]*val;
			...
			if (isNtrainingPow2 || i + blockSize < ntraining)
			{
				...
				reduction[tid]+=d_atraindata[bidy*ntraining +i+blockSize]*d_ytraindata[bidy*ntraining+i+blockSize]*val;
			}
		}
