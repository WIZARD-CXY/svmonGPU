			...
			int kernelcode)
{
		...
		printf("tesing Phase :: %llu bytes of memory found on device, %llu bytes currently free\n", totalMemory, remainingMemory);

		int sizeOfCache = remainingMemory/((int)rowPitch);

		sizeOfCache = (int)((float)sizeOfCache*0.80);
		...
		printf("testing Phase :: numThreads %i, numBlocksRed %i\n", numThreads, numBlocksRed);
