								...
								float d)
{
	...
	printf("training Phase :: %llu bytes of memory found on device, %llu bytes currently free %d bytes rowPitch\n", totalMemory, remainingMemory, rowPitch);

	int sizeOfCache = remainingMemory/rowPitch;
	sizeOfCache = (int)((float)sizeOfCache*0.90);
    ...
    printf("Training numThread :%d , numBlockRed :%d , activeTask: %d \n",numThreads, numBlocksRed, activeTasks);
	int smemSize = 0;
